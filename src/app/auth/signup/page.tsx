...
export default function SignupPage() {
  ...
  async function handleDevSignup() {
    setError('');
    setLoading(true);
    await signUp('demo@lifeos.app', 'devpassword');
    setLoading(false);
    router.push('/dashboard');
  }
      ...
      >
        <div className="rounded-2xl backdrop-blur-2xl bg-white/90 border border-gray-200/60 shadow-xl shadow-emerald-500/10 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <label className="mb-1.5 block text-sm font-medium text-gray-600">Confirm password</label>
              <div className="relative">
                <Lock size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  type={showPassword ? 'text' : 'password'}
          ...
          >
            ...
            <button
              ...
              className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-xs text-amber-400 hover:bg-amber-500/20 transition-all"
            >
              Quick demo login
            </button>
          </motion.div>
